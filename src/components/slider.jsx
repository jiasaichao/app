...
export class Root extends React.Component {
    ...
    render() {
        ...
        childrenItem.forEach((item, index) => {
            if (this.props.loop) {
                if (index === 0) {
                    content.push(<div key={'a' + index} style={{ overflow: 'hidden', width: bodywidth }}>{childrenItem[childrenItem.length - 1]}</div>)
                }
            }
            content.push(<div key={index} style={{ overflow: 'hidden', width: bodywidth }}>{item}</div>)
            if (this.props.loop) {
                if (index === childrenItem.length - 1) {
                    content.push(<div key={'a' + index} style={{ overflow: 'hidden', width: bodywidth }}>{childrenItem[0]}</div>)
                }
            }
        });
        /**指示当前第几页的小白点 */
